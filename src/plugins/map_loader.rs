        ...
        Err(err) => {
            ...
            match err {
                MinionError::MapFileNotFound { path } => {
                    warn!("Map file not found: {}. Check that the file exists and is readable.", path.display());
                }
                MinionError::InvalidMapData { reason } => {
                    warn!("Map data is invalid: {reason}. The file may be corrupted or from an incompatible version.");
                }
...
fn create_fallback_map_progressive(
    ...
    match error {
        MinionError::MapFileNotFound { path } => {
            ...
            warn!("SOLUTION: Create a map file at {} or check that the path in config is correct", path.display());
        ...
        MinionError::InvalidTerrainData { reason } => {
            info!("Invalid terrain data, using procedural terrain but keeping other map elements if possible");
...
fn create_generated_terrain_map(_game_config: &GameConfig) -> MinionResult<MapDefinition> {
    ...
    let terrain_generator = get_terrain_preset("hills", Some(42))
...
fn generate_terrain_based_spawn_zones(terrain: &TerrainData) -> MinionResult<Vec<SpawnZone>> {
    ...
    info!("Generated {} spawn zones using terrain analysis", spawn_zones.len());
    Ok(spawn_zones)
}
...
fn create_basic_spawn_zones(terrain: &TerrainData) -> MinionResult<Vec<SpawnZone>> {
    ...
    for (i, pos) in positions.into_iter().enumerate() {
        let zone = SpawnZone::new(
            ...
            1,
            vec!["dark-knight".to_string()],
        ).map_err(|e| MinionError::InvalidSpawnZoneData { 
