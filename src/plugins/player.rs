...
fn move_player(
    ...
    for (mut transform, mut player, mut controller, pathfinding_agent) in player_query.iter_mut() {
        ...
        let movement_target = pathfinding_waypoint.or(player.move_target);
        ...
        info!(
            "Movement decision: should_move={}, pathfinding_waypoint={:?}, movement_vector=({:.3}, {:.3}, {:.3})",
            should_move,
            pathfinding_waypoint.is_some(),
            final_movement_vector.x,
            final_movement_vector.y,
        ...
        if should_move {
            ...
            info!(
                "APPLYING MOVEMENT: vector=({:.2}, {:.2}, {:.2})",
                movement_with_gravity.x, movement_with_gravity.y, movement_with_gravity.z
            );
...
mod tests {
    ...
    fn test_pathfinding_agent_waypoint_reach_vs_movement_stopping() {
        ...
        // The waypoint_reach_distance should be smaller than stopping_distance to avoid conflicts
        assert!(
            pathfinding_agent.waypoint_reach_distance < movement_config.stopping_distance,
            "Waypoint reach distance should be smaller than movement stopping distance to avoid conflicts"
        );
