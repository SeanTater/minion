...
impl MovementDebugger {
    ...
    pub fn log_input_event(cursor_pos: Vec2, camera_pos: Vec3, ray_origin: Vec3, ray_direction: Vec3) {
        info!(
    ...
    pub fn log_target_calculation(player_pos: Vec3, player_y: f32, hit_point: Vec3, final_target: Vec3) {
        info!(
    ...
    pub fn log_movement_calculation(
        ...
        info!(
            ...
            calculation.movement_vector.x, calculation.movement_vector.y, calculation.movement_vector.z
        );
    }
    ...
    pub fn log_controller_output(entity: Entity, output: &KinematicCharacterControllerOutput) {
        info!(
            ...
            entity,
            output.effective_translation.x, output.effective_translation.y, output.effective_translation.z,
            output.grounded,
            output.desired_translation.x, output.desired_translation.y, output.desired_translation.z
        );
    }

    /// Log transform changes
    pub fn log_transform_change(entity: Entity, old_transform: Transform, new_transform: Transform) {
    ...
    pub fn log_rotation_change(entity: Entity, old_rotation: Quat, new_rotation: Quat, target: Option<Vec3>) {
    ...
    pub fn log_physics_state(entity: Entity, rigid_body: &RigidBody, velocity: Option<&Velocity>) {
        let vel_str = velocity
            .map(|v| format!("linear({:.2}, {:.2}, {:.2}) angular({:.2}, {:.2}, {:.2})",
    ...
    pub fn log_system_execution(system_name: &str, start_time: std::time::Instant) {
        ...
        debug!("TIMING: {} took {:.2}ms", system_name, elapsed.as_secs_f64() * 1000.0);
    }
    ...
    pub fn log_player_state_summary(
        ...
        let target_str = player.move_target
        ...
        let output_str = output
            .map(|o| format!("effective({:.3}, {:.3}, {:.3}) grounded({})",
                o.effective_translation.x, o.effective_translation.y, o.effective_translation.z,
                o.grounded))
            .unwrap_or_else(|| "None".to_string());
        ...
        info!(
            ...
            target_str,
            controller_translation.x, controller_translation.y, controller_translation.z,
            output_str
        );
    }
}
